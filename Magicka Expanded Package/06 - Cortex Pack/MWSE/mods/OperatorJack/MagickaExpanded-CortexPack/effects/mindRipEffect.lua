...
local function showCheckFailedTooltip(e)
    ...
    local label = tooltip:createLabel({
        text = "You are not intelligent enough to understand this spell."
    })
...
local function showCheckPassedTooltip(e)
    ...
    local label = tooltip:createLabel({
        text = string.format("Cost to learn: %spts", spell.magickaCost * 2)
    })
    label.borderLeft = 4
...
local function showUi(reference)
    ...
    if (#spells == 0) then
        tes3.messageBox("The target does not have any spells for you to take.")
        return
    end
